...
pathToSectionFiles = os.path.join('.', 'bladeSections')  # relative path, or can be absolute
pathToMeridGeomFiles = os.path.join('.', 'gasPath')  # relative path, or can be absolute
...
scriptFile = 'geomUpdate.sh'  # filename for script to modify blockMeshDict
...
for a in range(nSections):
    ...
    LE.append([fileP[0, 2], fileP[0, 0]])   # Leading edge (z, r)
    TE.append([fileP[-1, 2], fileP[-1, 0]]) # Trailing edge (z, r)
...
def objective_TEoffset(x):
    ...
    TEqplusSpline = spinterp.CubicSpline(
        TEqp[:,1], TEqp[:,0],
        bc_type=((1, 1/bisect_slope_SS_downstream), (1, 0))
    )
    TEqminusSpline = spinterp.CubicSpline(
        TEqm[:,1], TEqm[:,0],
        bc_type=((1, 0), (1, 1/bisect_slope_PS_downstream))
    )
...
# -----------------------------------------> Check )
    ...
    resTE = opt.minimize_scalar(objective_TEoffset, bounds=(0, 30), method='bounded')
    TEoffset = resTE.x
    best_scoreTE = resTE.fun
    ...
    TEqplusSpline = spinterp.CubicSpline(TEqp[:, 1],TEqp[:, 0],bc_type =
                            ((1, 1/bisect_slope_SS_downstream),(1, 0)))
    TEqminusSpline = spinterp.CubicSpline(TEqm[:, 1],TEqm[:, 0],bc_type =
                            ((1, 0),(1, 1/bisect_slope_PS_downstream))) # change to tangential direction at boundary
...
# --------------------------------------------------------------------------------------------------------------------------> Check )
    ...
    zPtsTE = np.concatenate((vqmPtsTE, vqpPtsTE))  # OK
    rPtsTE = np.concatenate((vmPtsTE, vpPtsTE))  # OK
    vPtsTE = np.concatenate((vqmPtsTEa, vqpPtsTEa))  # 
...
i = nSections // 2 # Index of the section to plot. You can change this to plot other sections.
...
plt.plot(
    z_allCamber[i, :], # Axial positions (z) along the camber line
    theta_allCamber[i, :] * r_allCamber[i, :], # Circumferential positions (r * theta)
    label='Original Camber' # Label for the original camber line
)

# Plot the camber line rotated by +halfPitch
plt.plot(
    z_pCamber[i, :], # Axial positions (z)
    theta_pCamber[i, :] * r_pCamber[i, :], # Circumferential positions (r * theta)
    label='Rotated +halfPitch' # Label for the camber line rotated by +halfPitch
)

# Plot the camber line rotated by -halfPitch
plt.plot(
    z_nCamber[i, :], # Axial positions (z)
    theta_nCamber[i, :] * r_nCamber[i, :], # Circumferential positions (r * theta)
    label='Rotated -halfPitch' # Label for the camber line rotated by -halfPitch
)
